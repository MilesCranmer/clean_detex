...
echo "Generating python 3 de-macro"
cat > /tmp/.de-macro.py <<- EOM
#!/usr/bin/python -O
...
def tokenize(in_str):
    ...
    cs.reset()
    if not cs.legal():
        raise SyntaxError("No string to tokenize.")
    while cs.uplegal():
        if "%" == cs.item:
...
class Tex_stream(Stream):
    ...
    def scan_group(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No group to scan.")
        item = self.item
    ...
    def scan_command_name(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No command name to scan.")
        item = self.item
        name = ""
        if item.type in [esc_symb_ty, esc_str_ty]:
            ...
            if not "{" == item.val:
                raise SyntaxError(SyntaxError("Command definition misses first {."))
            self.next()
            item = self.skip_blank_tokens()
            if not item.type in [esc_symb_ty, esc_str_ty]:
                raise SyntaxError("Command definition does not begin with control sequence.")
            name = item.val
            self.next()
            item = self.skip_blank_tokens()
            if not "}" == item.val:
                raise SyntaxError(("Definition for commmand %s misses first }., %s" % (name, item.val)))
        self.next()
    ...
    def scan_numargs(self, name):
        ...
        """
        if not self.legal():
            raise SyntaxError("No numargs to scan.")
        item = self.item
        numargs = 0
        if not simple_ty == item.type:
            raise SyntaxError("Illegal command or environment definition: "+name)
        if "[" == item.val:
            if not 4 < len(self.data):
                raise SyntaxError("Command or environment definition is illegal: "+name)
            item = self.next()
            if not simple_ty == item.type:
                raise SyntaxError("Illegal command or environment definition: "+name)
            numargs = item.val
            if not pos_digit_re.match(numargs):
                raise SyntaxError("%s must be argument number after %s" % (numargs, name))
            numargs = int(numargs)
            self.next()
            item = self.skip_blank_tokens()
            if not simple_ty == item.type:
                raise SyntaxError("Illegal command definition: "+name)
            if "]" != item.val:
                raise SyntaxError("Illegal command definition: "+name)
            self.next()
            self.skip_blank_tokens()
    ...
    def scan_command_def(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No command definition to scan.")
        item = self.item
        if not 2 < len(self.data):
            raise SyntaxError("Command definition is illegal.")
        # newcommand or renewcommand
        if not item.type in [esc_symb_ty, esc_str_ty]:
            raise SyntaxError("Command definition should begin with control sequence: "+item.val)
        if item.val not in ["newcommand", "renewcommand"]:
            raise SyntaxError("Command definition should begin with control sequence.")
        self.next()
        self.skip_blank_tokens()
        ...
        if group_ty != body_group.type:
            raise SyntaxError("Command body missing: "+cmd_name)
    ...
    def scan_env_name(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No environment name to scan.")
        item = self.item
        if not "{" == item.val:
            raise SyntaxError("Env. definition begins with %s, not with {" % (item.val))
        self.next()
        item = self.skip_blank_tokens()
        name = ""
        if not simple_ty == item.type:
            raise SyntaxError(("1. Env. def. begins with cont. seq. %s, not with env.name.")
                 % (item.val))
        while self.uplegal() and not blank_or_rbrace_re.match(item.val):
            name += item.val
            item = self.next()
            if not simple_ty == item.type:
                raise SyntaxError(("2. Env. def. begins with cont. seq. %s, not with env.name.")
                       % (item.val))
        item = self.skip_blank_tokens()
        if not "}" == item.val:
            raise SyntaxError("Command definition does not begin with control sequence.")
    ...
    def scan_env_def(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No environment definition to scan.")
        item = self.item
        if not 7 < len(self.data):
            raise SyntaxError("Environment definition is illegal.")
        pos = 0

        if not item.type in [esc_symb_ty, esc_str_ty]:
            raise SyntaxError(("Env. definition does not begin with control sequence:"+ item.val))
        if item.val not in ["newenvironment", "renewenvironment"]:
            raise SyntaxError("Env. definition does not begin with control sequence.")
        self.next()
        self.skip_blank_tokens()
        ...
        if group_ty != begin_group.type:
            raise SyntaxError("Begin body missing: "+env_name)
        ...
        if group_ty != end_group.type:
            raise SyntaxError("End body missing:"+env_name)
    ...
    def scan_defs(self):
        if not self.legal():
            raise SyntaxError("No definitions to scan.")
        self.reset()
    ...
    def scan_args(self, command_or_env_def):
        ...
        """
        if not self.legal():
            raise SyntaxError("No arguments to scan.")
    ...
    def scan_command(self, command_def):
        ...
        """
        if not self.legal():
            raise SyntaxError("No command to scan.")
        if not self.item.type in [esc_symb_ty, esc_str_ty]:
            raise SyntaxError("Command does not begin with control sequence.")
        name = self.item.val
        self.next()
    ...
    def scan_env_begin(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No environment begin to scan.")
        item = self.item
        if not (esc_str_ty == item.type and "begin" == item.val):
            raise SyntaxError("Environment does not begin with begin.")
        self.next()
    ...
    def scan_env_end(self):
        ...
        """
        if not self.legal():
            raise SyntaxError("No environment end to scan.")
        item = self.item
        if not (esc_str_ty == item.type and "end" == item.val):
            raise SyntaxError("Environment does not end with end.")
        self.next()
        name_group = self.scan_group()
        name = detokenize(name_group.val)
        return name
    ...
    def scan_env_rest(self, env_def):
        ...
        """
        if not self.legal():
            raise SyntaxError("No environment rest to scan.")
    ...
    def add_defs(self, defs_file):
        ...
        if not os.path.isfile(defs_file_compl):
            raise SyntaxError("%s does not exist" % (defs_file_compl))
    ...
    def subst_args(self, body, args):
        ...
        while pos < len(body):
            ...
            if not pos_digit_re.match(argnum):
                raise SyntaxError("# is not followed by number.")
            argnum = int(argnum)
            if argnum > len(args):
                raise SyntaxError("Too large argument number.")
            arg = args[argnum-1]
            out += arg
            pos += 1
        return out
    ...
    def apply_command_recur(self, command_instance):
        ...
        try:
            ...
            raise SyntaxError("apply_all_recur fails on command instance %s: %s, %s" % (command_instance.show(), detokenize(e.data), e.message))
        return result
    ...
    def apply_all_recur(self, data, report=False):
        ...
        if not ts.legal():
            raise SyntaxError(Empty_text_error(data, "No text to process."))
        while ts.uplegal():
    ...
    def process_file(self, file):
        ...
        if not self.data:
            raise SyntaxError("Empty tokenization result.")
        self.reset()
...
if [ "$DEMACRO" -eq "0" ]; then
    ...
    echo "Expanding user-defined macros. Make sure each newcommand is on a separate line. Don't redefine a command you have defined with newcommand."
    cat /tmp/.$OUTPUTFILENAME.detex1.3.tex | $VIMS -r 'newcommand' 'dd' > /tmp/.$OUTPUTFILENAME.macros2.detex-private.sty
    cat /tmp/.$OUTPUTFILENAME.detex1.3.tex | $VIMS -e 'newcommand' 'dd' -s '/usepackage\<enter>O\\usepackage{/tmp/.'"$OUTPUTFILENAME"'.macros2.detex}' > /tmp/.$OUTPUTFILENAME.macros2.detex.tex 
    python $DEMACROFILE /tmp/.$OUTPUTFILENAME.macros2.detex.tex 
    cp /tmp/.$OUTPUTFILENAME.macros2.detex-clean.tex /tmp/.$OUTPUTFILENAME.detex1.4.tex 
fi
